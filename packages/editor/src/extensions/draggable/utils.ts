...
export function removePossibleTable(view: EditorView,
  event: DragEvent): Transaction | null {
  const { state } = view
...
function nodeIsNotBlock(node: Node) {
...
function getDOMByPos(view: EditorView,
  root: HTMLElement,
  $pos: ResolvedPos) {
...
export function selectRootNodeByDom(dom: Element,
  view: EditorView): ActiveNode | null {
  const root = view.dom
  if (!root)
    return null
  if (dom === root)
