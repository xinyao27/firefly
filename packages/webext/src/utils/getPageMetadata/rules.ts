...
export const metaDataRules: Record<string, RuleSet> = {
  ...
  url: {
    ...
    defaultValue: context => context.options.url ?? '',
    processor: (url: any, context) => makeUrlAbsolute(context.options.url ?? '', url),
  },
  provider: {
    rules: [
    ...
    defaultValue: context => getProvider(parseUrl(context.options.url ?? '')),
  },
  keywords: {
    rules: [
  ...
  image: {
    ...
    processor: (imageUrl: any, context) => context.options.forceImageHttps === true ? makeUrlSecure(makeUrlAbsolute(context.options.url ?? '', imageUrl)) : makeUrlAbsolute(context.options.url ?? '', imageUrl),
  },
  icon: {
    rules: [
    ...
    defaultValue: context => makeUrlAbsolute(context.options.url ?? '', '/favicon.ico'),
    processor: (iconUrl, context) => context.options.forceImageHttps === true ? makeUrlSecure(makeUrlAbsolute(context.options.url ?? '', iconUrl)) : makeUrlAbsolute(context.options.url ?? '', iconUrl),
  },
  video: {
    rules: [
